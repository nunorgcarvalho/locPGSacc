...
H2 <- 0.1 # how much variance is explained by genetics
...
C0 <- sqrt((var_additive + B2^2 * var_GxE_base) * ((1-H2)/H2) )
...
full_data$additive_effect <- (as.matrix(full_data[,paste0("g",1:M)]) %*% B1)[,1]
full_data$Y <- B0 + (full_data$additive_effect) + (B2 * full_data$g1 * full_data$E1) + (C0 * full_data$err)

#ggplot(full_data, aes(x=E1, y=Y, color=as.factor(g1))) + geom_point(alpha=0.05) + geom_smooth(method="lm")
